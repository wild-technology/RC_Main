...
class BatchDirectory(RCModule):
	...
	# Camera configuration mapping for EXIF injection
	CAMERA_CONFIGS = {
		'camupper': {
			'make': 'ZCAM',
			'model': 'F6 8-15mm Fisheye Upper',
			'focal_length': 13
		},
		'cammid': {
			'make': 'ZCAM',
			'model': 'F6 8-15mm Fisheye Mid',
			'focal_length': 13.5
		},
		'camlower': {
			'make': 'ZCAM',
			'model': 'F6 16-35mm III Lower',
			'focal_length': 16
		},
		'zeuss': {
			'make': 'Zeuss',
	...
	def get_parameters(self) -> dict[str, Parameter]:
		...
		additional_params['batch_inject_exif'] = Parameter(
			name='Inject Camera EXIF',
			cli_short='b_e',
			cli_long='b_inject_exif',
			type=bool,
			default_value=True,
			description='Inject camera model EXIF metadata during copy',
			prompt_user=False
		)
	...
	def __scott_bandwidth(xy: np.ndarray) -> float:
		"""
		Calculate bandwidth using Scott's rule: σ × n^(-1/(d+4))
		where σ is standard deviation, n is sample size, d is dimensions
		"""
		n, d = xy.shape
		if n < 2:
			return 1.0
	...
	def __inject_camera_exif(self, input_path, output_path, camera_subfolder):
		"""
		Inject camera EXIF metadata based on camera subfolder.

		Args:
			input_path: Source image path
			output_path: Destination image path
			camera_subfolder: Camera subfolder identifier

		Returns:
			bool: True if EXIF was successfully injected, False otherwise
		"""
		if camera_subfolder not in self.CAMERA_CONFIGS:
			shutil.copy2(input_path, output_path)
			return False

		config = self.CAMERA_CONFIGS[camera_subfolder]
		filename = os.path.basename(input_path)

		try:
			# Check if file is JPEG (piexif.insert only works with JPEG)
			...
			focal_rational = (int(config['focal_length'] * 10), 10)
			exif_dict["Exif"][piexif.ExifIFD.FocalLength] = focal_rational

			# Dump EXIF bytes and copy file
			...
			if written_focal != focal_rational:
				raise ValueError(f"EXIF verification failed: Focal length mismatch")

			return True

		except Exception as e:
			self.logger.error(f"EXIF injection failed for {filename}: {type(e).__name__}: {e}")
	...
	def __copy_files(
		...
		"""
		Copy files to camera-specific subfolders with optional EXIF injection.
		"""
		if self._file_index is None:
			self._file_index = self.__build_file_index(input_dir)

		inject_exif = self.params.get('batch_inject_exif', Parameter('', '', '', bool, True)).get_value()
		...
		for file in files:
			try:
				...
				if os.path.exists(output_path):
					...
					if inject_exif and camera_subfolder in self.CAMERA_CONFIGS:
						stats['exif_attempted'] += 1
						exif_success = self.__inject_camera_exif(file_path, output_path, camera_subfolder)
						if exif_success:
							stats['exif_injected'] += 1
						else:
							stats['exif_failed'] += 1
					else:
		...
		if inject_exif:
			self.logger.info(f"  EXIF attempted: {stats['exif_attempted']}")
			self.logger.info(f"  EXIF injected successfully: {stats['exif_injected']}")
			self.logger.info(f"  EXIF injection failed: {stats['exif_failed']}")
	...
	def __create_batch_folders(self, output_dir, zones, input_dir, flight_log_path=None):
		...
		for i, zone_files in enumerate(zones):
			...
			aggregate_stats['exif_injected'] += zone_stats['exif_injected']
			aggregate_stats['exif_attempted'] += zone_stats['exif_attempted']
			aggregate_stats['exif_failed'] += zone_stats['exif_failed']
		...
		self.logger.info(f"  Total EXIF injections attempted: {aggregate_stats['exif_attempted']}")
		self.logger.info(f"  Total EXIF injections successful: {aggregate_stats['exif_injected']}")
		self.logger.info(f"  Total EXIF injections failed: {aggregate_stats['exif_failed']}")
		if aggregate_stats['exif_attempted'] > 0:
			success_rate = (aggregate_stats['exif_injected'] / aggregate_stats['exif_attempted']) * 100
			self.logger.info(f"  EXIF success rate: {success_rate:.1f}%")
	...
	def run(self):
		...
		try:
			...
			return {
				...
				'EXIF Injections Successful': batch_stats['exif_injected'],
				'EXIF Injections Failed': batch_stats['exif_failed'],
