...
### Cross-Cutting Concerns (Apply to Every Sprint)

**File Naming Convention**: ALL generated files include `{expedition}_{dive}_{utm_zone}` prefix.
Example: `NA173_H2102_57L_zone_001_20250705_0347.rsalign`

**Console Logging**: Every file processed logged with exact path + operation + result.
Format: `[timestamp] [MODULE] Processing: /full/path/to/file.ext (step N/M) — result`

**Input Validation**: Every module validates input formats in `validate_parameters()`:
- Flight logs: semicolon delimiter, required columns, non-empty rows
- ROV CSV: tab delimiter, required kalman_* columns
- XML params: `<Configuration>` root, `<entry key="..." value="..."/>` children
- Images: file exists, PIL.Image.verify() passes
- Paths: exist, writable, sufficient disk space

**Save/Progress/Variables**: Each sprint must include:
- Session state JSON save after step completion
- Checkpoint file for resumable operations
- All RC parameters stored in RealityScan XML format (`<Configuration id="..."><entry key="..." value="..."/>`)
- Variables passed between modules via the params dict AND persisted to session JSON

**XML Parameter Format**: All XML files follow RealityScan's format exactly:
```xml
<Configuration id="{GUID}">
  <entry key="parameterName" value="parameterValue"/>
</Configuration>
```
New parameter files generated by the system must use this schema.

**Testing Strategy (Linux Development, Windows Target)**:
- Unit tests run on Linux with mock subprocess (no RC needed)
- `@pytest.mark.windows_only` for tests requiring RC instance
- Test fixtures in `tests/fixtures/` with sample flight logs, ROV CSV, RC status outputs, XML params, and deliberately malformed variants for validation testing
- Claude Code runs in this Linux environment — it cannot directly control a Windows machine or run RC. Tests that exercise delegation are mocked. The user runs Windows integration tests locally.

---

### Sprint 1: Shared Infrastructure (Phase 1)
**Agent**: general-purpose (code generation)
- Create `modules/rc_common/` package:
  - `rc_delegation.py` — RCDelegationClient with two-phase detection, on_progress callback
  - `rc_status.py` — RCStatusParser (tested against `tests/fixtures/sample_rc_status_outputs.txt`)
  - `progress.py` — ProgressReporter with TqdmBackend + LogBackend
  - `camera_utils.py` — detect_camera_type(), load_camera_profiles(), get_camera_profile()
  - `file_validators.py` — validate_flight_log(), validate_rov_csv(), validate_rc_xml(), validate_image()
  - `naming.py` — generate_filename(expedition, dive, utm_zone, ...) for enforcing naming convention
  - `session.py` — SessionState save/load (JSON), CheckpointManager
  - `rc_xml.py` — read/write RealityScan XML format, generate new configs with proper `<Configuration>` schema
- Create `config/camera_profiles.json` (resolved values from user decisions)
- Modify `module_base/rc_module.py` — Add ProgressReporter, session state hooks
- Modify `module_base/parameter.py` — Add parameter_group, min_value, max_value, choices, file_filter fields
- Tests: `test_rc_status_parser.py`, `test_file_validators.py`, `test_camera_utils.py`, `test_naming.py`, `test_session.py`, `test_rc_xml.py`

### Sprint 2: Image Enhancement (Phase 2)
**Agent**: general-purpose
- Create `modules/image_enhancement/` package
- Register in main.py
- Input validation: verify image files, check output dir writable
- Output filenames: `{expedition}_{dive}_{utm}_enhanced_{original_name}`
- Console logging: each file processed with before/after stats
- Save: session state updated with enhancement parameters and file list
- Tests: `test_image_enhancement.py` with tiny test images

### Sprint 3: Enhanced Alignment (Phase 4)
**Agent**: general-purpose
- Add delegation mode to `realitycapture_interface.py`
- Add batch sequential alignment from BatchSequentialAlignment.py
- Checkpoint: save after each zone alignment (zone name, project path, timestamp)
- Resume: skip completed zones on checkpoint load
- Console logging: exact RC command delegated, status polling output, per-zone results
- Input validation: verify flight log format, image directory structure, RC executable exists
- Save: session state with per-zone results, checkpoint file per long operation
- All component exports named: `{expedition}_{dive}_{utm}_zone_{N}_{timestamp}_{component}.rsalign`
- Tests: `test_alignment_delegation.py` with mock subprocess
...
- Register in main.py
- Camera setup reads from `config/camera_profiles.json`
- Console logging: per-camera-group settings applied, image counts per group
- Component export: revision-based detection, exact file paths logged
- Output filenames: `{expedition}_{dive}_{utm}_{component_name}.rsalign`
- Save: session state with camera settings applied, components exported
- Input validation: verify camera_profiles.json format, verify RC instance responsive
- Tests: `test_camera_setup.py`, `test_component_export.py` with mocks

### Sprint 5: Model Generation (Phase 6)
**Agent**: general-purpose
- Create `modules/model_generation/` package
- Rearrangeable pipeline steps with dependency validation
- All thresholds as configurable parameters
- Signal handling: SIGINT → abort_instance → save → exit
- Console logging: each pipeline step with duration, triangle counts, texture sizes
- Output filenames: `{expedition}_{dive}_{utm}_{component}_HighPoly.fbx`, etc.
- Save: checkpoint after each component, session state with per-component results
- RC parameter XMLs generated in RealityScan format
- Input validation: verify .rsalign files exist, export dir writable
- Tests: `test_model_generation.py` with mock delegation client

### Sprint 6: main.py Integration (Phase 7)
**Agent**: general-purpose
- Update module registration (8 modules)
- Add global parameters (rc_executable_path, rc_instance_name, camera_profiles_path)
- Automatic re-georeferencing after batching (transparent)
- Session save/load from main.py level
- Console logging harmonized across all modules
- End-to-end CLI testing with test fixtures
- Input validation at pipeline level (cross-module dependency checks)
- Tests: `test_main_integration.py` with mocked modules

### Sprint 7: GUI Foundation (Phase 8)
**Agent**: general-purpose
- PySide6 main window + wizard-style step panels
- Parameter form auto-generation from Parameter objects (using parameter_group, choices, min/max)
- Log viewer widget (QPlainTextEdit, color-coded, auto-scroll, filter buttons)
- Progress widget (progress bar + elapsed + ETA)
- Stats table widget (QTableWidget)
- Expedition/dive entry on startup → propagates to all modules
- Save: session state save/load from File menu
- Tests: `test_gui_widgets.py` with QTest

### Sprint 8: GUI Workers + State (Phase 8 cont.)
**Agent**: general-purpose
- Pipeline worker (QRunnable → runs modules in background thread)
- RC process communication (QProcess or threaded subprocess)
- Session save/load dialog (File → Save Session / Load Session)
- Checkpoint management UI (load checkpoint, show completed steps)
- Metadata DB (SQLite: expeditions, dives, sensors, sessions)
- Progress signals: worker → main thread → progress widget + log viewer
- Tests: `test_gui_workers.py`, `test_session_roundtrip.py`

### Sprint 9: GUI Panels + Polish
**Agent**: general-purpose
- Step-specific panels with statistics tables (per-step stats from Phase 8.4)
- File/process statistics display after each step
- Expedition/dive management UI
- Drag-and-drop model pipeline step reordering
- PyInstaller packaging for Windows
- Tests: `test_gui_panels.py`, manual testing checklist
