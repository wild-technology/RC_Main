...
**Pipeline flow**: Extract Images → Enhance Images → Georeference → Batch → Camera Setup → RealityCapture Alignment → Component Export → Model Generation
...
### File Naming
ALL generated files MUST include `{expedition}_{dive}_{utm_zone}` in the filename.
...
- Flight logs: Check semicolon delimiter, required column headers, row count
...
### Flight Log (semicolon-delimited, generated by Georeference step)
```
